...
class XTemplate {
	...
	private $_null_string = array('' => '');             /* null string for unassigned vars */
	private $_null_block = array('' => '');  /* null string for unassigned blocks */
	private $_error = '';
	private $_autoreset = true;                                     /* auto-reset sub blocks */

	private $_ignore_missing_blocks = true ;          // NW 17 oct 2002 - Set to FALSE to
	// generate errors if a non-existant blocks is referenced

	// JC 20/11/02 for echoing the template filename if in development
	private $_file_name_full_path = '';
	...
	function parse ($bname) {
		...
		foreach ($var_array as $k => $v) {
			...
			if ($sub[0] == '_BLOCK_') {
				...
				$public = isset($this->parsed_blocks[$bname2]) ? $this->parsed_blocks[$bname2] : null;
				...
				if ($public == '') {
					...
					$public = str_replace('\\|', '|', $public);
					$copy = preg_replace("|" . $this->tag_start_delim . $v . $this->tag_end_delim . "|", "$public", $copy);
				}
			} else {
				...
				foreach ($sub as $v1) {
					...
					//                if (empty($public[$v1])) { // this line would think that zeros(0) were empty - which is not true
					if (!isset($public[$v1]) || (!is_array($public[$v1]) && strlen($public[$v1]) == 0)) {

						// Check for constant, when variable not assigned
						if (defined($v1)) {

							$public[$v1] = constant($v1);
				...
				$public = (!isset($public)) ? $nul : $public;
     ...
     * @param string $public Variable to assign values to
     * @param string / array $value Value to assign to $public
    */
	function insert_loop ($bname, $public, $value = '') {

		$this->assign($public, $value);
		$this->parse($bname);
	}
     ...
     * @param string $public Variable to assign values to
     * @param array $values Values to assign to $public
    */
	function array_loop ($bname, $public, &$values) {

		if (is_array($values)) {

			foreach($values as $v) {

				$this->assign($public, $v);
				$this->parse($bname);
			}
		}
	}
	...
	function text ($bname = '') {
		...
		{
			echo "text[{$i}] => '{$text[$i]}' [" . ord($text[$i]) . "] <br/>";
		}
		*/
     ...
     */
	private function _maketree ($con, $parentblock='') {
     ...
     * @return array
     */
	private function _store_filevar_parents ($blocks){
     ...
     * @return string
     */
	private function _r_getfile ($file) {
     ...
     */
	private function _pre_var_dump () {
